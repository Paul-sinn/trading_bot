  ...
  return (
    <Card className="space-y-3">
      ...
      <div className="rounded-md border border-neutral-800 bg-neutral-900 px-3 py-2 text-xs text-amber-400">
        Discord approval triggers execution only after all gates are rechecked.
      </div>
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm sm:grid-cols-3">
        ...
        <Item
          ...
          tone={
            status?.latest_decision === "REAL_READY_DRY_RUN" ||
            status?.latest_decision === "APPROVED_READY_DRY_RUN"
              ? "warn"
              : "muted"
          }
        />
        <Item label="최근 승인 id" value={status?.latest_approval_id ?? "—"} mono />
        <Item label="주문유형" value={status?.latest_order_type ?? "—"} />
        <Item label="broker_order_id" value={status?.latest_broker_order_id ?? "—"} mono />
        <Item
          label="test/proof 이력"
