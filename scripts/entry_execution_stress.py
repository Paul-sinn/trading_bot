"""진입 실행 슬리피지 + 갭 스트레스 러너 — 3% limit + next-open 두 시뮬을 돌려 스트레스 비교를 만든다.
...
from agents.execution_stress import compute_execution_stress, format_execution_stress  # noqa: E402
...
def _config_to_args(settings, model, buffer) -> SimpleNamespace:
    return SimpleNamespace(
        ...
        entry_fill_model=model, entry_limit_buffer_pct=buffer, weekend_exit_symbols=[],
    )
...
def run_execution_stress(
    *, data_root, benchmark="SPY", symbols=None, events_csv="data/events.csv",
    ...
    """3% limit + next-open 시뮬 두 번 → compute_execution_stress. 데이터 실패는 (None, error)."""
    ...
    report = compute_execution_stress(limit_diag, nopen_diag, price_data, starting_cash=starting_cash)
    return report, None


def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="진입 실행 슬리피지+갭 스트레스(3% limit vs next-open, 실주문 0)")
...
def run(args) -> int:
    report, error = run_execution_stress(
    ...
    if report is None:
        ...
        raise SystemExit(2)
    text = format_execution_stress(report)
    print(text)
    if args.output:
