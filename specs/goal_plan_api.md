...
## 의존성 주입(DI)
- `get_goal_plan_provider()` → 기본 `MockGoalPlanProvider`(외부 호출 없음, 후속 phase에서 Claude).
- `get_portfolio_provider_dep()` → 기존 `get_portfolio_provider()`를 인자 없이 감싼 thin wrapper.
  (그 `settings: Settings` 파라미터가 FastAPI body 필드로 노출돼 요청 body가 잘못 embed되는 것을 막는다.)
- `get_session_factory()` → 모듈 레벨 세션 팩토리(미설정 시 `make_session_factory()`로 지연 생성).
  이 프로젝트는 flat route 등록(`routes.extend`)을 써서 `app.dependency_overrides`가 동작하지 않으므로,
  테스트는 `set_session_factory(<인메모리 팩토리>)`로 주입하고 끝나면 `set_session_factory(None)`으로
  복원한다(파일 DB 오염 금지 — ADR-004).
