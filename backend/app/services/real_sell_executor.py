"""수동 확인 매도 워커 v0 **scaffold** — 기본 비활성, 실 매도 없음.

기존 Agentic 포지션에 대한 매도 readiness만 판정한다. 현재 단계엔 실 매도 경로가 결선돼 있지 않다:
...
CRITICAL 불변식(이 task):
- 매도 주문 없음. 모든 receipt: `broker_order_id=None`, `real_sell_order_placed=False`,
  `real_sell_orders_placed=0`, `real_order_placed=False`, `real_orders_placed=0`(매수 카운터 불변).
...
class RealSellExecutionDisabled(RuntimeError):
    """실 매도 경로가 비활성/미결선임을 알리는 예외(scaffold). 실 MCP write 미도달 보장(fail-closed)."""
...
class RealRobinhoodSellExecutor:
    ...
    실제 MCP 주문 도구(place_equity_order sell 등)를 import/호출하지 않는다.
    """

    name = "real_robinhood_sell"

    def submit_limit_sell(self, *, symbol: str, quantity: float, limit_price: float) -> dict:
        raise RealSellExecutionDisabled(
            "real sell execution path is not wired (scaffold). No Robinhood write tool is reachable."
        )
...
class RealSellExecutionReceipt(BaseModel):
    ...
    def model_post_init(self, _context) -> None:
        ...
        # 실 매도 흔적은 SELL_SUBMITTED(실제 제출)에만 보존. 그 외 모두 0/false 강제.
...
def process_sell_execution(
    ...
    *,
    ...
    """단일 매도 intent를 평가해 영수증을 append한다(매도 주문 없음)."""
